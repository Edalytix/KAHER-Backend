  ...
  return Object.freeze({
    ...
    findReports: async ({
      ...
      try {
        ...
        if (
          filterValues.pendingFinance ||
          filterValues.pendingRegistrar ||
          filterValues.pendingPrincipal
        ) {
          const resApplications = [];
          let filter = '';
          if (filterValues.pendingFinance) {
            filter = 'Finance';
          } else if (filterValues.pendingPrincipal) {
            filter = 'Principal';
          } else {
            filter = 'Registrar';
          }
          application.forEach((element) => {
            if (
              ...
              element.workflow.approvals[element.currentApprover - 1] &&
              element.workflow.approvals[
                element.currentApprover - 1
              ].name.includes(filter)
            ) {
              resApplications.push(element);
            }
          });
          application = resApplications;
        }
    ...
    findLastApplication: async (userUID, workflowUID) => {
      try {
        const application = await Application.findOne({
          user: userUID,
    ...
    findAllAssignedApps: async (id, searchQuery = '', statusQuery) => {
      try {
        ...
        const archiveApplications = await Application.aggregate([
          {
            $lookup: {
              from: 'workflows',
              localField: 'workflow',
        ...
        const populatedArchiveApplications = await Application.populate(
          archiveApplications,
          [
            { path: 'user' },
            { path: 'department' },
        ...
        return {
          msg: `Find applications result`,
          data: { populatedApplications, populatedArchiveApplications },
        };
      } catch (error) {
        if (error instanceof CreateError) {
          throw error;
    ...
    institutionReport: async (userUID) => {
      try {
        let institutions = await Institution.find();
        const institutionArray = [];
    ...
    workflowReport: async (userUID) => {
      try {
        let workflows = await Workflow.find({ version: 'latest' });
        const workflowArray = [];
    ...
    applicationApprovedAmountReport: async (userUID) => {
      try {
        let workflows = await Workflow.find({ version: 'latest' });
        const workflowArray = [];
        ...
        for (let index = 0; index < workflows.length; index++) {
          ...
          workflowArray.push({
            name: element.name,
            approvedAmount: totalApplications[0]?.totalApprovedAmount || 0,
          });
        }
        return {
          msg: `Find applications result`,
    ...
    applicationStatusReport: async (userUID) => {
      try {
        let totalApplications = await Application.aggregate([
          {
            $group: {
        ...
        delete totalApplications[0]?._id;
        return {
          msg: `Find applications result`,
