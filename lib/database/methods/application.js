...
exports.Application = ({ translate, logger, CreateError, lang }) => {
  return Object.freeze({
    ...
    findByParams: async (params) => {
      try {
        let applications = await Application.find(params);
        return {
    ...
    findReports: async ({
      ...
      try {
        ...
        if (filterValues.formId) {
          const formId = new mongoose.Types.ObjectId(filterValues.formId);
          pipeline.push({
            $match: { 'workflow.forms.form._id': formId },
          });
        }
    ...
    findAllAssignedApps: async (id, searchQuery = '', statusQuery) => {
      try {
        ...
        console.log(departmentIds, roleId);
