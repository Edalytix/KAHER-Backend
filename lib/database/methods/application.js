...
exports.Application = ({ translate, logger, CreateError, lang }) => {
  return Object.freeze({
    ...
    findAll: async (page, limit, searchQuery='') => {
      try {
        let  applications = await Application.find({title: { $regex: searchQuery, $options: 'i' } })
          .populate('user')
          .populate('department')
          .populate('workflow')
          .exec();
      ...
      findAllOfUsers: async (page, limit, uid,searchQuery='') => {
        try {
          let applications = await Application.find({user: uid,title: { $regex: searchQuery, $options: 'i' } })
                  .populate('user')
          .populate('department')
          .populate('workflow')
          .exec();
    ...
    findById: async (id) => {
      ...
      findUsers: async (id,searchQuery) => {
        try {
        ...
        findAllAssignedApps: async (id,searchQuery='') => {
          try {
