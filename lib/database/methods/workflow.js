...
exports.Workflow = ({ translate, logger, CreateError, lang }) => {
  return Object.freeze({
    ...
    findAll: async (page, limit,searchQuery='') => {
      try {
        let workflows = await Workflow.find({name: { $regex: searchQuery, $options: 'i' } });
    ...
    update: async ({ id, params }) => {
      try {
        ...
        const update = await Workflow.findByIdAndUpdate(id, Params.updateParams, {new: true})
        return {
    ...
    addForms: async ({ id, params }) => {
      try {
        const Params = load(params);
        const update = await Workflow.findByIdAndUpdate(id, { forms: Params.updateParams.forms})
