...
exports.Department = ({ translate, logger, CreateError, lang }) => {
  return Object.freeze({
    ...
    findAllNoAuthorization: async () => {
      try {
        let departments = await Department.find({}, { name: 1, _id: 1 });
      ...
      try {
        ...
        department.users.forEach(async (element) => {
          const updateUser = await User.findByIdAndUpdate(
            { _id: element },
            { $set: { department: { name: null, id: null } } }
          );
        });
        const res = await Department.deleteOne({ _id: id });
        if (res.deletedCount !== 1) {
          throw Error('Department not deleted.');
        }
        return {
    ...
    findById: async (id) => {
      try {
        ...
        return {
          ...
          data: { department: { ...department._doc }, ...res[0] },
        };
      } catch (error) {
        if (error instanceof CreateError) {
          throw error;
      ...
      try {
        ...
        Params.updateParams.users.forEach(async (element) => {
          ...
          if (user.department.id !== id) {
            const removeUser = await Department.findByIdAndUpdate(
              user.department.id,
              {
                $pull: { users: element },
              }
    ...
    addApplications: async ({ id, params }) => {
      try {
        const Params = load(params);
        const update = await Department.findByIdAndUpdate(id, {
          $set: { applications: Params.updateParams.applications },
        });
