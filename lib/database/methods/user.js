...
exports.User = ({ translate, logger, CreateError, lang }) => {
  return Object.freeze({
    ...
    findByDepartmentRole: async (params) => {
      try {
    ...
    addApplicationForNonPrincipal: async ({
      roleID,
      departmentID,
      applicationID,
    }) => {
    ...
    addApplicationForPrincipal: async ({
      roleID,
      departmentID,
      applicationID,
      institutionID,
    ...
    approverReport: async (user) => {
      try {
        const objectIdArray = user.applications;

        const institutionApplications = await Application.aggregate([
          {
            $lookup: {
              from: 'users',
              localField: 'user',
          ...
          {
            $match: {
              'user.institution': user.institution._id,
            },
          },
          {
            $match: {
