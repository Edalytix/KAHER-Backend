...
exports.Form = ({ translate, logger, CreateError, lang }) => {
  return Object.freeze({
    ...
    findAll: async (page, limit,searchQuery='') => {
      try {
        let forms = await Form.find({title: { $regex: searchQuery, $options: 'i' } });
    ...
    deleteById: async (id) => {
      try {
        console.log(id)
        const workflows = await Workflow.find({ forms:  { $elemMatch: { form: id } }});
        ...
        for (const workflow of workflows) {
          ...
          workflowDoc.forms = workflowDoc.forms.filter(formId => formId != id);
