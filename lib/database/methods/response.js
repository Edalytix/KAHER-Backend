...
// Loop through the array and delete each property from the comment object
        for (let i = 0; i < propertiesToDelete.length; i++) {
          const propertyName = propertiesToDelete[i];
          delete updateParams[propertyName];
        }
   
        const update = await Response.findByIdAndUpdate({fuid: Params.updateParams.fuid,
          uid: Params.updateParams.uid,
          auid: Params.updateParams.auid,
          wuid: Params.updateParams.wuid,}, updateParams)
