...
exports.Response = ({ translate, logger, CreateError, lang }) => {
  return Object.freeze({
    ...
    findByField: async (params) => {
      try {
        let response = await Response.find(params);
        return {
          msg: `Find result`,
          data: response,
        };
        ...
        logger.error('Failed to find: %s %s', error.message, error);
        throw Error(translate(lang, 'unknown_error_contact_support'));
      }
    },
    ...
    update: async (params) => {
      try {
        ...
        const propertiesToDelete = ['fuid', 'auid', 'wuid', 'uid', 'ruid'];
        const updatedParams = {};
